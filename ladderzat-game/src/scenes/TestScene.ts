...
export default class TestScene extends Phaser.Scene {
	...
	addUi() {
		...
		this.input.on('gameobjectover', function (pointer, gameObject) {
			if (gameObject && gameObject.name && gameObject.name.startsWith("button-dpad")) {
				...
				console.debug('DURECTUIN: ' + gameObject.dirName);
		...
		var p = this.player;
		var jump = this.add.sprite(this.WIDTH - 12, this.HEIGHT + this.TOP_OFFSET - 12, 'button-jump').setOrigin(1, 1).setInteractive().setName('button-jump');
	...
	createPlayer() {
		...
		var player = this.physics.add.sprite(86, -50, 'kris-stand');
	...
	addImage(name: string, x: number, y: number): Image {
		let image = this.add.image(x, y + this.TOP_OFFSET, name);
		image.setOrigin(0, 0);
		return image;
	}
